...
\begin{frame}[shrink=5]{Papel Central das Proteínas}
\begin{center}
...
\begin{frame}[shrink=10]{Termodinâmica do Dobramento Proteico}
...
\begin{frame}[shrink=15]{Diagrama de Ramachandran}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=15]{Domínios e Motivos Proteicos}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=15]{Proteínas Intrinsecamente Desordenadas (IDPs)}
...
\begin{frame}[shrink=10]{Protein Data Bank (PDB)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[fragile, shrink=10]{Análise de Estruturas com BioPython}
\begin{lstlisting}[language=Python, caption=Parser de arquivos PDB, basicstyle=\ttfamily\tiny]
...
\begin{frame}[fragile, shrink=5]{Visualização de Proteínas com PyMOL}
\begin{lstlisting}[language=Python, caption=Script PyMOL para visualização, basicstyle=\ttfamily\tiny]
...
\begin{frame}[shrink=5]{7.2 Cinética Enzimática - Revisão de Michaelis-Menten}
...
\begin{frame}[shrink=5]{Gráfico de Eadie-Hofstee}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=10]{Ligação Cooperativa e Equação de Hill}
...
\begin{frame}{Regulação Alostérica: Modelo MWC (1/2)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}{Regulação Alostérica: Modelo KNF (2/2)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=10]{Inibição Enzimática: Tipos}
...
\begin{frame}[shrink=10]{Inibição Enzimática: Tipos Adicionais}
...
\begin{frame}[shrink=15]{Inibição Reversível vs Irreversível}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[fragile, shrink=10]{Ajuste de Dados de Cinética Enzimática}
\begin{lstlisting}[language=Python, caption=Regressão não-linear para Michaelis-Menten, basicstyle=\ttfamily\tiny]
import numpy as np
...
\begin{frame}[fragile, shrink=15]{Cálculo de IC50 e Ki}
\begin{lstlisting}[language=Python, caption=Determinação de IC50, basicstyle=\ttfamily\tiny]
from scipy.optimize import curve_fit
...
\begin{frame}[shrink=10]{7.3 Sinalização Celular: Visão Geral (1/2)}
...
\begin{block}{Etapas da transdução}
\begin{itemize}
    \item Ligante extracelular é reconhecido por um receptor
    \item O receptor ativa transdutores intracelulares
    \item O sinal chega a efetores e gera resposta celular
\end{itemize}
\end{block}
\end{frame}

\begin{frame}[shrink=10]{7.3 Sinalização Celular: Visão Geral (2/2)}
\begin{center}
...
\begin{frame}[shrink=5]{Tipos de Receptores de Membrana (1/2)}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{GPCRs}
...
\begin{frame}[shrink=5]{Tipos de Receptores de Membrana (2/2)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=10]{Segundos Mensageiros}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{cAMP}
\textbf{AMP cíclico}
...
\begin{frame}[shrink=20]{Cascata MAPK: Via Clássica}
\begin{center}
...
\begin{frame}[shrink=10]{MAPK: Módulo de Amplificação (1/2)}
\importante{
...
\begin{frame}[shrink=10]{MAPK: Módulo de Amplificação (2/2)}
...
\begin{frame}[shrink=10]{Dinâmica de Fosforilação e Desfosforilação}
...
\begin{frame}[fragile, shrink=15]{Modelagem de Cascatas de Sinalização}
\begin{lstlisting}[language=Python, caption=Cascata MAPK simplificada, basicstyle=\ttfamily\tiny]
import numpy as np
...
\begin{frame}[shrink=10]{7.4 Modificações Pós-Traducionais (PTMs) (1/2)}
\definicao{PTM}{
...
\begin{frame}[shrink=10]{7.4 Modificações Pós-Traducionais (PTMs) (2/2)}
\importante{
PTMs regulam:
\begin{itemize}
...
\begin{frame}[shrink=5]{Fosforilação: PTM mais Estudada}
\begin{block}{Características}
\begin{itemize}
...
\begin{frame}[shrink=5]{Acetilação e Metilação: Regulação Epigenética}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=10]{Ubiquitinação e Degradação Proteica}
...
\begin{frame}[shrink=10]{Glicosilação: Modificação Mais Complexa}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=10]{Crosstalk entre PTMs}
\importante{
...
\begin{frame}[shrink=10]{Bancos de Dados de PTMs}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=5]{PTMs no Controle do Ciclo Celular (1/2)}
...
\begin{frame}[shrink=5]{PTMs no Controle do Ciclo Celular (2/2)}
\begin{block}{Principais Efeitos}
\begin{itemize}
...
\begin{frame}[shrink=10]{7.5 Proteômica: Estudo do Proteoma}
\definicao{Proteoma}{
...
\begin{frame}[shrink=5]{Abordagens Proteômicas}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=20]{Espectrometria de Massa: LC-MS/MS}
\begin{center}
...
\begin{frame}[shrink=10]{Técnicas de Ionização}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{ESI}
...
\begin{frame}[shrink=10]{Quantificação Proteômica (1/2)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=10]{Quantificação Proteômica (2/2)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=5]{Interações Proteína-Proteína}
...
\begin{frame}[shrink=10]{Bancos de Dados de Interações}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{STRING}
...
\begin{frame}[fragile, shrink=20]{Análise de Dados Proteômicos}
\begin{lstlisting}[language=Python, caption=Processamento de dados MS, basicstyle=\ttfamily\tiny]
import pandas as pd
import numpy as np
...
\begin{frame}[fragile, shrink=15]{Visualização: Volcano Plot (1/2)}
\begin{lstlisting}[language=Python, caption=Volcano plot proteômico, basicstyle=\ttfamily\tiny]
...
\begin{frame}[fragile, shrink=15]{Visualização: Volcano Plot (2/2)}
\begin{lstlisting}[language=Python, caption=Volcano plot proteômico, basicstyle=\ttfamily\tiny]
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 1: Cinética Enzimática}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 2: Estrutura e Análise}
...
\begin{frame}{Exercícios - Parte 3: Sinalização}
...
\begin{frame}{Exercícios - Parte 3: PTMs}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 4: Proteômica}
...
\begin{frame}[allowframebreaks]{Referências Complementares}
\begin{thebibliography}{99}
...
\begin{frame}[allowframebreaks]{Leitura Complementar e Recursos}
