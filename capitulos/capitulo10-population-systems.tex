...
\destaque{
$\displaystyle \derivada{N}{t} = \underbrace{bN}_{\text{nascimento}} - \underbrace{dN}_{\text{morte}} + \underbrace{I}_{\text{imigração}} - \underbrace{E}_{\text{emigração}}$
}
\end{frame}
...
\destaque{
$\displaystyle R_0 = \frac{\beta}{\gamma} = \beta \times \frac{1}{\gamma} = \text{(taxa de contato)} \times \text{(duração da infecção)}$
}
