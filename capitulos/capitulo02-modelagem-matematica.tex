...
\begin{frame}[shrink=10]{Modelos Empíricos vs Mecanísticos}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[fragile, shrink=5]{Implementação: Decaimento Radioativo (1/2)}
\begin{lstlisting}[language=Python, caption=Definição e parâmetros]
import numpy as np
...
\begin{frame}[shrink=5]{Classificação de EDOs}
...
\begin{frame}[shrink=10]{Exemplo: Crescimento Logístico}
...
\begin{frame}[fragile, shrink=5]{Implementação: Crescimento Logístico (1/2)}
\begin{lstlisting}[language=Python, caption=Definição e Resolução]
import numpy as np
import matplotlib.pyplot as plt
from scipy.integrate import odeint
...
\begin{frame}[shrink=10]{Exemplo: Modelo Predador-Presa (Lotka-Volterra)}
\begin{block}{Sistema}
\begin{align*}
...
\begin{frame}[fragile, shrink=5]{Implementação: Lotka-Volterra (1/2)}
\begin{lstlisting}[language=Python, caption=Sistema Lotka-Volterra]
import numpy as np
import matplotlib.pyplot as plt
from scipy.integrate import odeint
...
\begin{frame}[fragile, shrink=15]{Retrato de Fase}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=10]{Método de Euler}
...
\begin{frame}[fragile, shrink=10]{Implementação do Método de Euler}
...
\begin{frame}[shrink=10]{Método de Runge-Kutta de 4ª Ordem (RK4)}
\begin{block}{Ideia}
...
\begin{frame}[fragile, shrink=10]{Implementação do RK4}
\begin{lstlisting}[language=Python, caption=Runge-Kutta 4, basicstyle=\ttfamily\tiny]
def rk4(f, x0, t):
...
\begin{frame}[shrink=10]{Analise de Sensibilidade}
...
\begin{frame}[shrink=5]{Tipos de Analise de Sensibilidade}
\begin{block}{Local}
\begin{itemize}
...
\begin{frame}[fragile, shrink=15]{Analise de Sensibilidade Local}
\begin{lstlisting}[language=Python, caption=Sensibilidade por diferenças finitas, basicstyle=\ttfamily\tiny]
...
\begin{frame}[shrink=10]{Estimação de Parâmetros}
...
\begin{frame}[fragile, shrink=15]{Ajuste de Curvas com scipy}
\begin{lstlisting}[language=Python, caption=Estimação de parâmetros, basicstyle=\ttfamily\tiny]
...
\begin{frame}[shrink=5]{Validação de Modelos}
...
\begin{frame}[shrink=10]{Estudo de Caso: Dinâmica de Infecção Viral}
\begin{block}{Sistema}
...
\begin{frame}[shrink=10]{Numero Básico de Reprodução}
...
\begin{frame}[fragile, shrink=5]{Implementação do Modelo SIR (1/2)}
\begin{lstlisting}[language=Python, caption=Definição e Parâmetros]
...
\begin{frame}[fragile, shrink=5]{Implementação do Modelo SIR (2/2)}
\begin{lstlisting}[language=Python, caption=Simulação]
# Simular
t = np.linspace(0, 200, 1000)
sol = odeint(sir_model, y0, t, args=(beta, gamma))

# Resultado
print(f"Populacao total: {N}")
print(f"R0: {R0}")
\end{lstlisting}
\end{frame}

\begin{frame}[fragile, shrink=10]{Analise e Visualização}
\begin{lstlisting}[language=Python, caption=Analise do modelo SIR, basicstyle=\ttfamily\tiny]
S, I, R = sol.T
plt.figure(figsize=(10, 6))
...
\begin{frame}[shrink=10]{Projeto: Modelo SIR com Vacinação}
\begin{alertblock}{Extensão do Modelo}
...
\begin{frame}[shrink=10]{Leitura Complementar}
