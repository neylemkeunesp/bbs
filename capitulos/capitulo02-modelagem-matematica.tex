...
% ============================================================================
% SEÇÃO 11: MODELOS ESTOCÁSTICOS E REPTATION
% ============================================================================
\section{Modelos Estocásticos: Reptation do DNA}

\begin{frame}[shrink=10]{Por que Modelos Estocásticos?}
\begin{block}{Limitações dos Modelos Determinísticos}
\begin{itemize}
    \item Assumem grande número de moléculas e variáveis contínuas
    \item Ignoram flutuações térmicas e variabilidade molecular
    \item Inadequados quando $N \lesssim 100$ moléculas ou eventos raros
\end{itemize}
\end{block}

\vspace{0.3cm}

\begin{block}{Quando usar modelos estocásticos?}
\begin{itemize}
    \item Movimentos de moléculas individuais
    \item Sistemas com ruído térmico relevante (escala nanométrica)
    \item Polímeros em confinamento (gel, cromatina, poros)
    \item Expressão gênica com poucos fatores de transcrição
\end{itemize}
\end{block}
...
\begin{frame}{Movimento de Polímeros: O Modelo de Reptation}
\definicao{Reptation}{
...
\textbf{Ideia central:}
\begin{itemize}
    \item O DNA não pode atravessar os obstáculos da rede
    \item Cria um ``tubo'' ao longo de seu contorno
    \item Move-se pela criação/destruição de alças nas extremidades
    \item Movimento resultante: difusão ao longo do próprio eixo
\end{itemize}
...
\begin{frame}{Formulação Estocástica do Reptation (1/2)}
\begin{block}{Modelo de Caminhada Aleatória no Tubo}
O DNA é representado por $N$ segmentos de comprimento $b$. A cada passo de tempo $\Delta t$, cada extremidade:
\begin{itemize}
    \item Com prob.\ $\frac{1}{2}$: \textbf{avança} — adiciona um segmento à frente
    \item Com prob.\ $\frac{1}{2}$: \textbf{recua} — remove um segmento da frente
\end{itemize}
\end{block}

\vspace{0.4cm}

\textbf{Coeficiente de difusão curvilinear:}
\destaque{
\begin{equation*}
D_c = \frac{b^2}{2\,\tau}
\end{equation*}
...
\begin{frame}{Formulação Estocástica do Reptation (2/2)}
\textbf{Coeficiente de difusão efetivo (3D):}
\begin{equation*}
...
\begin{exampleblock}{Predição chave}
O tempo de renovação do tubo escala como $\tau_{\rm rep} \sim N^3$ — verificado experimentalmente por eletroforese em gel.
\end{exampleblock}
...
\begin{frame}[shrink=15]{Reptation em Campo Elétrico (Eletroforese)}
\begin{block}{Biased Reptation (Duke, 1989)}
Na presença de campo elétrico $\mathbf{E}$, a probabilidade de avanço/recuo nas extremidades se torna assimétrica:
\begin{align*}
...
\textbf{Mobilidade eletroforética:}
\begin{equation*}
...
\column{0.5\textwidth}
\begin{itemize}
    \item Fragmentos menores: maior mobilidade
    \item Base da separação por gel eletroforese
    \item Válido para DNA \textbf{longo} (regime de reptation)
\end{itemize}
\column{0.45\textwidth}
\begin{alertblock}{Limite do modelo}
Para campos altos ($qEb \gg k_BT$): DNA se alinha com o campo e a separação por tamanho se perde.
\end{alertblock}
\end{columns}
\end{frame}

\begin{frame}[fragile, allowframebreaks]{Implementação: Simulação de Reptation}
...
\begin{frame}[fragile, shrink=5]{Implementação: Análise e Visualização}
...
\begin{frame}[shrink=5]{Predições e Verificação Experimental}
\begin{block}{Escalonamentos do Modelo de Reptation}
\begin{center}
\begin{tabular}{lll}
\hline
\textbf{Grandeza} & \textbf{Predição} & \textbf{Verificação} \\
\hline
Difusividade & $D \sim N^{-2}$ & Eletroforese em campo nulo \\
Mobilidade & $\mu \sim N^{-1}$ & Gel de agarose, campo baixo \\
Tempo de renovação & $\tau \sim N^3$ & Birrefringência transitória \\
MSD curvilinear & $\langle s^2 \rangle \sim t$ & FCS, tracking de moléculas \\
\hline
\end{tabular}
\end{center}
\end{block}
...
\begin{exampleblock}{Aplicações Biotecnológicas}
\begin{itemize}
    \item \textbf{Eletroforese em gel de agarose:} separação de fragmentos de DNA por tamanho
    \item \textbf{Sequenciamento de DNA:} entender mobilidade de fragmentos
    \item \textbf{Eletroforese em campo pulsado (PFGE):} separação de cromossomos inteiros
    \item \textbf{Nanoporos:} translocação de DNA — extensão do modelo de reptation
\end{itemize}
\end{exampleblock}
\end{frame}

% ============================================================================
% SEÇÃO 12: BOAS PRÁTICAS
% ============================================================================
