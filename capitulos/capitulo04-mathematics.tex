...
\begin{frame}[allowframebreaks]{Visão Integrada: Do Modelo à Predição}
\begin{center}
...
\column{0.5\textwidth}
\begin{block}{Parte III: Análise e Modelagem Discreta}
\begin{enumerate}
    \setcounter{enumi}{6}
    \item Análise de sensibilidade
    \item Modelos recursivos determinísticos
    \item Processos estocásticos discretos
\end{enumerate}
\end{block}
...
\begin{itemize}
    ...
    \item Mapa Logístico e Caos
    \item Cadeias de Markov
    \item Processo de Galton-Watson
\end{itemize}
\end{exampleblock}
\end{columns}
\end{frame}
...
\begin{frame}[allowframebreaks]{Revisão: Equações Diferenciais Ordinárias}
...
\begin{frame}[allowframebreaks]{Campos Vetoriais e Retratos de Fase}
...
\begin{frame}[allowframebreaks]{Classificação de Pontos Fixos em 2D}
\begin{center}
\begin{tikzpicture}[scale=0.9]
    % Eixos
...
\begin{frame}[allowframebreaks]{Tipos de Pontos Fixos}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[allowframebreaks]{Espirais: Estáveis vs. Instáveis}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[allowframebreaks]{Bifurcação Saddle-Node (Fold)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[allowframebreaks]{Bifurcação Transcrítica}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{Equação Normal}
\begin{equation*}
...
\begin{frame}[allowframebreaks]{Bifurcação Pitchfork}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[allowframebreaks]{Bifurcação de Hopf}
...
\begin{frame}[allowframebreaks]{Soluções Periódicas}
...
\begin{frame}[allowframebreaks]{Ciclos Limite}
...
% ============================================================================
% SEÇÃO 7: MODELOS DISCRETOS E RECURSIVOS
% ============================================================================
...
\begin{frame}[allowframebreaks]{Cadeias de Markov de Tempo Discreto}
\definicao{Cadeia de Markov Discreta (DTMC)}{
Processo estocástico onde a probabilidade do próximo estado depende estritamente e apenas do estado atual (ausência de memória):
\begin{equation*}
...
Dada a matriz de transição $\vect{P}$ e o vetor de probabilidade de estados $\vect{p}(t)$ no tempo $t$:
\begin{equation*}
\vect{p}(t+1) = \vect{P} \cdot \vect{p}(t) \quad \Rightarrow \quad \vect{p}(t) = \vect{P}^t \vect{p}(0)
\end{equation*}
\end{block}

\begin{exampleblock}{Aplicações em Biologia}
\begin{itemize}
    \item \textbf{Canais Iônicos}: Transição estocástica entre estados (aberto, fechado, inativo).
    \item \textbf{Evolução Molecular}: Modelos de mutação de nucleotídeos (ex: Jukes-Cantor).
\end{itemize}
\end{exampleblock}
\end{frame}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 1: EDOs}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 2: Estabilidade}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 3: Bifurcações}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 4: Oscilações}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 5: Sensibilidade}
...
\begin{frame}[allowframebreaks]{Exercícios - Parte 6: Modelos Discretos}
