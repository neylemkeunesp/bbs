...
\begin{block}{Propriedades da Matriz de Transição $\vect{P}$}
\begin{itemize}
    \item Elementos são probabilidades: $P_{ij} \ge 0$.
    \item Matriz Estocástica por Linha: $\sum_{j} P_{ij} = 1$ (soma de cada linha é 1).
\end{itemize}
\end{block}

\definicao{Distribuição Estacionária ($\vect{\pi}$)}{
A distribuição estacionária é o vetor de probabilidades $\vect{\pi}$ que permanece invariante após um passo de transição:
\begin{equation*}
\vect{\pi} = \vect{P}^T \vect{\pi} \quad \text{ou} \quad \pi_j = \sum_{i} \pi_i P_{ij}
\end{equation*}
sujeito a $\sum_{i} \pi_i = 1$.
}

\begin{exampleblock}{Comportamento de Longo Prazo}
Para cadeias ergódicas (irredutíveis e aperiódicas), qualquer condição inicial converge para a distribuição estacionária: $\lim_{t \to \infty} \vect{p}(t) = \vect{\pi}$.
\end{exampleblock}
\end{frame}

\begin{frame}{Cadeias de Markov: Exemplo de Canal Iônico}
Considere um canal iônico simplificado com dois estados: \textbf{Fechado (F)} e \textbf{Aberto (A)}.
...
\end{align*}
$\pi_A$ representa a fração de canais abertos no equilíbrio!
\end{columns}

\vspace{0.4cm}
\begin{center}
\begin{tikzpicture}[node distance=2.2cm, auto, scale=0.8, transform shape]
    \node[circle, draw, fill=laranjacel!20, minimum size=1cm] (F) {Fechado (F)};
    \node[circle, draw, fill=azulclaro!20, minimum size=1cm, right of=F] (A) {Aberto (A)};
    \path[->, thick, >=stealth]
        (F) edge [bend left] node {$\alpha$} (A)
        (A) edge [bend left] node {$\beta$} (F)
        (F) edge [loop left] node {$1-\alpha$} (F)
        (A) edge [loop right] node {$1-\beta$} (A);
\end{tikzpicture}
\end{center}
\end{frame}
