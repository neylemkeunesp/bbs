...
% ============================================================================
\title[Cap. 4: Mathematics of Biological Systems]{Capítulo 4: The Mathematics of Biological Systems}
...
\begin{frame}[shrink=5]{Visão Geral do Capítulo}
...
\begin{frame}[shrink=8]{Matemática como Linguagem da Biologia}
...
\begin{frame}[shrink=10]{Visão Integrada: Do Modelo à Predição}
\begin{center}
\begin{tikzpicture}[node distance=1.5cm, scale=0.85, transform shape,
    box/.style={rectangle, draw, fill=azulclaro!20, text width=3cm, text centered, rounded corners, minimum height=0.8cm}]

    \node[box, fill=verdeacido!20] (bio) {Sistema Biológico};
    \node[box, fill=laranjacel!20, below of=bio] (model) {Modelo Matemático\\(EDOs)};
    \node[box, fill=roxodna!20, below of=model] (analysis) {Análise\\(Estabilidade, Bifurcações)};
    \node[box, fill=azulclaro!30, below of=analysis] (sim) {Simulação\\Numérica};
    \node[box, fill=verdeacido!30, below of=sim] (pred) {Predição \&\\Validação};

    \draw[->, thick, azulescuro] (bio) -- (model) node[midway, right, font=\tiny] {abstração};
    \draw[->, thick, azulescuro] (model) -- (analysis) node[midway, right, font=\tiny] {matemática};
    \draw[->, thick, azulescuro] (analysis) -- (sim) node[midway, right, font=\tiny] {computação};
    \draw[->, thick, azulescuro] (sim) -- (pred) node[midway, right, font=\tiny] {interpretação};
    \draw[->, thick, verdeacido, bend right=60] (pred) to node[left, font=\tiny] {refinamento} (bio);

\end{tikzpicture}
\end{center}
\end{frame}

\begin{frame}[shrink=5]{Tópicos Avançados deste Capítulo}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{Parte I: Dinâmica}
\begin{enumerate}
    \item Sistemas de EDOs
    \item Retratos de fase
    \item Análise de estabilidade
    \item Linearização e Jacobiano
\end{enumerate}
\end{block}

\begin{block}{Parte II: Transições}
\begin{enumerate}
    \setcounter{enumi}{4}
    \item Bifurcações
...
\column{0.5\textwidth}
\begin{block}{Parte III: Análise}
\begin{enumerate}
    \setcounter{enumi}{6}
    \item Análise de sensibilidade
    \item Exploração de espaço paramétrico
\end{enumerate}
\end{block}
...
\begin{itemize}
    \item Lotka-Volterra
    \item Van der Pol
    \item Modelos de regulação gênica
    \item Osciladores biológicos
\end{itemize}
\end{exampleblock}
\end{columns}
\end{frame}
...
% ============================================================================
% SEÇÃO 2: SISTEMAS DE EQUAÇÕES DIFERENCIAIS
% ============================================================================
\section{Sistemas de Equações Diferenciais}

\begin{frame}[shrink=8]{Revisão: Equações Diferenciais Ordinárias}
...
\begin{frame}[shrink=5]{Sistemas de EDOs Não-Lineares}
...
\begin{frame}[shrink=8]{Campos Vetoriais e Retratos de Fase}
...
\begin{itemize}
    ...
    \item Soluções seguem as linhas de fluxo
    \item Trajetórias formam o retrato de fase
\end{itemize}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \node[below] at (0,-2.5) {Oscilador Harmônico};
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}

\begin{frame}[shrink=8]{Nulclinas}
\definicao{Nulclina}{
...
\begin{frame}[shrink=8]{Campos de Direção}
...
\begin{frame}[shrink=5]{Exemplo: Modelo Lotka-Volterra}
...
\begin{frame}{Retrato de Fase do Lotka-Volterra}
\begin{columns}
\column{0.5\textwidth}
...
% ============================================================================
% SEÇÃO 3: ANÁLISE DE ESTABILIDADE
% ============================================================================
\section{Análise de Estabilidade}

\begin{frame}[shrink=5]{Pontos de Equilíbrio}
...
\begin{frame}[shrink=5]{Análise de Estabilidade Linear}
...
\begin{frame}[shrink=5]{Autovalores e Estabilidade}
\importante{
...
\begin{frame}[shrink=8]{Classificação de Pontos Fixos em 2D}
\begin{center}
\begin{tikzpicture}[scale=0.9]
    % Eixos
    \draw[->] (-0.5,0) -- (5,0) node[right] {$\text{tr}(\vect{J})$};
    \draw[->] (0,-0.5) -- (0,4) node[above] {$\det(\vect{J})$};

    % Parabola: tr^2 = 4*det
    \draw[thick, dashed] plot[domain=0:4] (\x, {\x*\x/4});

    % Regioes
    \fill[azulclaro!20] (0,0) -- (0,4) -- plot[domain=0:4] (\x, {\x*\x/4}) -- cycle;
    \node at (1,2.5) {\small Espirais};

    \fill[verdeacido!20] plot[domain=0:4] (\x, {\x*\x/4}) -- (4,0) -- cycle;
    \node at (3,1) {\small Nós};

    % Linha det = 0
    \draw[thick] (0,0) -- (5,0);
    \node[below] at (4,-0.3) {\small Selas};

    % Linha tr = 0
    \draw[thick] (0,0) -- (0,4);

    % Anotacoes
    \node[left, font=\tiny] at (-0.3,2) {Estável};
    \node[right, font=\tiny] at (5,2) {Instável};
    \node[above, font=\tiny] at (1,3.8) {$\text{tr}^2 < 4\det$};
    \node[below, font=\tiny, rotate=45] at (3.5,3) {$\text{tr}^2 > 4\det$};

    % Pontos exemplo
    \fill[red] (-1.5,1) circle (2pt);
    \node[left, font=\tiny] at (-1.5,1) {Espiral estável};

    \fill[red] (1.5,1) circle (2pt);
    \node[right, font=\tiny] at (1.5,1) {Espiral instável};
\end{tikzpicture}
\end{center}
\end{frame}

\begin{frame}[shrink=10]{Tipos de Pontos Fixos}
\begin{columns}
\column{0.5\textwidth}
...
\begin{itemize}
    ...
    \item Instável (pelo menos uma direção diverge)
\end{itemize}
\begin{center}
\begin{tikzpicture}[scale=0.5]
    \draw[->] (-2,0) -- (2,0);
...
\begin{frame}[shrink=8]{Espirais: Estáveis vs. Instáveis}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=5]{Teorema de Estabilidade de Lyapunov}
...
\begin{frame}[shrink=8]{Exemplo: Análise de Estabilidade}
\textbf{Sistema:}
\begin{align*}
...
\begin{frame}[shrink=5]{O que são Bifurcações?}
...
\begin{frame}[shrink=8]{Bifurcação Saddle-Node (Fold)}
\begin{columns}
\column{0.5\textwidth}
...
\textbf{Análise:}
\begin{itemize}
    \item $\mu < 0$: dois equilíbrios\\
    $(x^* = \pm\sqrt{-\mu})$
    \item $\mu = 0$: um equilíbrio\\
    $(x^* = 0)$ - bifurcação!
    \item $\mu > 0$: nenhum equilíbrio
\end{itemize}
...
\textbf{Interpretação:}
\begin{itemize}
    \item Colisão e aniquilação de equilíbrios
    \item Transição para divergência
\end{itemize}
...
\begin{tikzpicture}[scale=0.9]
    ...
    \node[right, font=\small] at (1,1) {sem equilíbrio};

    % Legendas
    \node[below, font=\tiny] at (0,-2.5) {Linha sólida: estável};
    \node[below, font=\tiny] at (0,-2.8) {Linha tracejada: instável};
\end{tikzpicture}
\end{center}
\end{columns}
...
\begin{frame}[shrink=8]{Bifurcação Transcrítica}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{Equação Normal}
\begin{equation*}
...
\textbf{Estabilidade:}
\begin{itemize}
    \item $\mu < 0$: $x=0$ estável, $x=\mu$ instável
    \item $\mu > 0$: $x=0$ instável, $x=\mu$ estável
    \item $\mu = 0$: troca de estabilidade!
\end{itemize}

\textbf{Interpretação:}
\begin{itemize}
    \item Equilíbrios trocam de estabilidade
    \item Comum quando há conservação
\end{itemize}
...
\begin{tikzpicture}[scale=0.9]
    ...
    \draw[thick, azulescuro, dashed, domain=-2:0] plot (\x, \x);
    \draw[thick, azulescuro, domain=0:2] plot (\x, \x);
...
Transição de crescimento populacional quando recurso disponível muda
\end{columns}
\end{frame}

\begin{frame}[shrink=10]{Bifurcação Pitchfork}
\begin{columns}
\column{0.5\textwidth}
...
\begin{itemize}
    \item $x^* = 0$
    \item $x^* = \pm\sqrt{\mu}$ (se $\mu > 0$)
\end{itemize}

\textbf{Estabilidade:}
\begin{itemize}
    \item $\mu < 0$: só $x=0$ (estável)
    \item $\mu > 0$: $x=0$ instável, $x=\pm\sqrt{\mu}$ estáveis
\end{itemize}
...
\begin{tikzpicture}[scale=0.7]
    ...
    \draw[thick, azulescuro, domain=0:1.5] plot (\x, {-sqrt(\x)});
    \fill[red] (0,0) circle (2pt);
    \node[below] at (0,-1.8) {Supercrítica};
\end{tikzpicture}
\end{center}
...
\textbf{Estabilidade:}
\begin{itemize}
    \item $\mu < 0$: $x=0$ estável, $x=\pm\sqrt{-\mu}$ instáveis
    \item $\mu > 0$: $x=0$ instável (divergência)
\end{itemize}
...
\begin{frame}[shrink=8]{Bifurcação de Hopf}
...
\begin{itemize}
    ...
    \item $\frac{d\alpha}{d\mu}\bigg|_{\mu_c} > 0$
\end{itemize}
...
\textbf{Tipos:}
\begin{itemize}
    \item \textbf{Supercrítica:} ciclo limite estável emerge
    \item \textbf{Subcrítica:} ciclo limite instável colapsa
\end{itemize}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \begin{scope}[xshift=-2cm]
        ...
        \node[below] at (0,-1) {$\mu < \mu_c$};
        \node[below] at (0,-1.3) {\tiny estável};
    \end{scope}
    ...
    \begin{scope}[xshift=2cm]
        ...
        \draw[->, thick] (0,0.4) arc (90:135:0.4);
        \node[below] at (0,-1) {$\mu > \mu_c$};
        \node[below] at (0,-1.3) {\tiny oscilação};
    \end{scope}
\end{tikzpicture}
\end{center}

\textbf{Amplitude do ciclo:}
\begin{equation*}
A \sim \sqrt{\mu - \mu_c}
\end{equation*}
\end{columns}
...
\begin{frame}[shrink=10]{Diagramas de Bifurcação: Resumo Visual}
\begin{center}
\begin{tikzpicture}[scale=0.6]
    % Saddle-node
...
% ============================================================================
% SEÇÃO 5: OSCILAÇÕES E CICLOS LIMITE
% ============================================================================
\section{Oscilações e Ciclos Limite}

\begin{frame}[shrink=5]{Soluções Periódicas}
...
\begin{itemize}
    ...
    \item \textbf{Relaxação:} fast-slow dynamics
\end{itemize}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[verdeacido, ->] (0.8,0) arc[start angle=0, end angle=60, radius=0.8];
    \draw[verdeacido, ->] (2,0) arc[start angle=0, end angle=60, radius=2];

    \node[below] at (0,-2.5) {Ciclo Limite};
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}

\begin{frame}[shrink=8]{Ciclos Limite}
...
\begin{itemize}
    \item Centro: família de órbitas
    \item Ciclo limite: órbita única
\end{itemize}
...
\begin{frame}[shrink=5]{Teorema de Poincaré-Bendixson}
...
\begin{frame}[shrink=8]{Oscilador de Van der Pol}
...
\begin{frame}[shrink=8]{Retratos de Fase do Van der Pol}
\begin{center}
\begin{tikzpicture}[scale=0.7]
...
\begin{frame}[shrink=8]{Osciladores Biológicos}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[shrink=5]{Análise de Sensibilidade Local}
...
\begin{frame}[shrink=5]{Coeficientes de Sensibilidade}
...
\begin{frame}[shrink=5]{Equações de Sensibilidade}
...
\begin{frame}[shrink=8]{Análise de Sensibilidade Global}
...
\begin{frame}[shrink=8]{Exploração do Espaço Paramétrico}
...
\begin{frame}[shrink=8]{Exemplo: Sensibilidade de Modelo Cinético}
...
\begin{frame}[shrink=10]{Exercícios - Parte 1: EDOs}
...
\begin{frame}[shrink=5]{Exercícios - Parte 2: Estabilidade}
...
\begin{frame}[shrink=5]{Exercícios - Parte 3: Bifurcações}
...
\begin{frame}[shrink=5]{Exercícios - Parte 4: Oscilações}
...
\begin{frame}[shrink=5]{Exercícios - Parte 5: Sensibilidade}
...
\begin{frame}[shrink=3]{Recursos Computacionais}
