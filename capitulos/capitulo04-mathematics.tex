...
\begin{frame}{Matemática como Linguagem da Biologia}
...
\begin{tikzpicture}[node distance=2cm, scale=0.9, transform shape,
    box/.style={rectangle, draw, fill=azulclaro!20, text width=3cm, text centered, rounded corners, minimum height=1cm}]
...
\begin{frame}{Tópicos Avançados deste Capítulo}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}{Sistemas de EDOs Não-Lineares}
...
\begin{tikzpicture}[scale=0.8]
    ...
    % Trajetória circular
    \draw[verdeacido, thick, ->] (1,0) arc (0:270:1);
...
\begin{frame}[fragile]{Resolvendo EDOs com SciPy}
...
\begin{frame}{Exemplo: Modelo Lotka-Volterra}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[->, thick] (2.5,2) arc (0:45:0.5 and 0.4);
    \draw[->, thick] (3,2) arc (0:45:1 and 0.8);
\end{tikzpicture}
\end{center}
\textbf{Interpretação:}\\
...
\begin{frame}[fragile]{Simulando Lotka-Volterra}
...
\begin{frame}{Pontos de Equilíbrio}
...
\begin{frame}{Autovalores e Estabilidade}
\importante{
...
\begin{frame}{Tipos de Pontos Fixos}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}{Teorema de Estabilidade de Lyapunov}
...
\begin{frame}[fragile]{Calculando Jacobiano e Autovalores com Python}
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize]
import numpy as np
...
\begin{frame}{Exemplo: Análise de Estabilidade}
\textbf{Sistema:}
\begin{align*}
...
\begin{frame}{O que são Bifurcações?}
...
\begin{frame}{Bifurcação Saddle-Node (Fold)}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}{Bifurcação Transcrítica}
\begin{columns}
\column{0.5\textwidth}
\begin{block}{Equação Normal}
\begin{equation*}
...
\begin{frame}{Bifurcação Pitchfork}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}{Bifurcação de Hopf}
...
\begin{frame}{Diagramas de Bifurcação: Resumo Visual}
\begin{center}
\begin{tikzpicture}[scale=0.6]
    % Saddle-node
...
\begin{frame}[fragile]{Gerando Diagramas de Bifurcação com Python}
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize]
import numpy as np
import matplotlib.pyplot as plt
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[verdeacido, ->] (0.8,0) arc (0:60:0.8);
    \draw[verdeacido, ->] (2,0) arc (0:60:2);
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[->, thick, red] (1.2,0) arc (0:30:1.2);
...
\begin{frame}{Teorema de Poincaré-Bendixson}
...
\begin{frame}{Retratos de Fase do Van der Pol}
\begin{center}
\begin{tikzpicture}[scale=0.7]
    ...
    \begin{scope}[xshift=0cm]
        ...
        \draw[->, thick] (1.5,0) arc (0:30:1.5 and 1.8);
        \fill[red] (0,0) circle (2pt);
...
\begin{frame}[fragile]{Simulando Van der Pol}
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize]
import numpy as np
from scipy.integrate import odeint
import matplotlib.pyplot as plt
...
\begin{frame}{Osciladores Biológicos}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}{Análise de Sensibilidade Local}
...
\begin{frame}{Coeficientes de Sensibilidade}
...
\begin{frame}{Equações de Sensibilidade}
...
\begin{frame}{Análise de Sensibilidade Global}
...
\begin{frame}{Exploração do Espaço Paramétrico}
...
\begin{frame}{Exemplo: Sensibilidade de Modelo Cinético}
...
\begin{frame}[fragile]{Calculando Sensibilidade com Python}
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize]
import numpy as np
from scipy.integrate import odeint
...
\begin{frame}[fragile]{Análise Global com SALib}
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize]
...
\begin{frame}{Exercícios - Parte 1: EDOs}
...
\begin{frame}{Exercícios - Parte 2: Estabilidade}
...
\begin{frame}{Exercícios - Parte 3: Bifurcações}
...
\begin{frame}{Exercícios - Parte 4: Oscilações}
...
\begin{frame}{Exercícios - Parte 5: Sensibilidade}
