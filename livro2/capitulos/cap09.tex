...
\begin{tikzpicture}[scale=0.9, transform shape,
    ...
    \node[box, fill=laranjacel!30] (rtk) {RTK};
    ...
    \draw[->, thick] (rtk) -- (grb);
    \draw[->, thick] (grb) -- node[right, font=\footnotesize] {ativação} (ras);
    \draw[->, thick] (ras) -- (raf);
    \draw[->, thick] (raf) -- node[right, font=\footnotesize] {P} (mek);
    \draw[->, thick] (mek) -- node[right, font=\footnotesize] {P\,P} (erk);
    \draw[->, thick] (erk) -- (tf);

    \node[below of=tf, node distance=1.0cm, text width=3.2cm, align=center, font=\footnotesize, fill=verdeacido!15, rounded corners]
        (resp) {\textbf{Resposta:}\\Proliferação\\Diferenciação\\Sobrevivência};
    \draw[->, thick] (tf) -- (resp);
\end{tikzpicture}
\caption{Cascata canônica MAPK/ERK. A ativação de um receptor tirosina quinase (RTK) na membrana inicia uma cadeia de fosforilação sequencial: Grb2/SOS ativa a GTPase Ras, que ativa Raf (MAPKKK), que fosforila MEK (MAPKK), que fosforila ERK (MAPK). ERK ativada transloca para o núcleo e fosforila fatores de transcrição, gerando respostas de proliferação, diferenciação ou sobrevivência. Cada passo da cascata amplifica o sinal --- tipicamente 10 a 100 vezes --- e a fosforilação dupla de ERK (P\,P) serve como um \emph{switch} digital.}
\label{fig:09-mapk}
\end{figure}
...
\begin{exemploenumv}[Wnt em câncer colorretal]
