...
\begin{tikzpicture}[scale=0.85]
    ...
    \begin{scope}[xshift=0cm]
        ...
        \node[below, font=\small] at (1,-0.3) {Primária};
        \node[above, font=\footnotesize] at (1,0.3) {Sequência};
    \end{scope}

    % Estrutura secundária (hélice)
    \begin{scope}[xshift=3.5cm]
        \draw[thick, azulescuro, decorate, decoration={coil, aspect=0.6, segment length=3mm, amplitude=2mm}] (0,0) -- (2,0);
        \node[below, font=\small] at (1,-0.3) {Secundária};
        \node[above, font=\footnotesize] at (1,0.3) {$\alpha$-hélice};
    \end{scope}

    % Estrutura terciária (dobrada)
    \begin{scope}[xshift=7cm]
        \draw[thick, azulescuro, rounded corners=5pt] (0,0) -- (0.5,0.3) -- (1.0,0.1) -- (1.5,0.4) -- (2,0);
        \node[below, font=\small] at (1,-0.3) {Terciária};
        \node[above, font=\footnotesize] at (1,0.3) {Dobrada};
    \end{scope}

    % Estrutura quaternária (2 subunidades)
    \begin{scope}[xshift=10.5cm]
        \draw[thick, verdeacido, fill=verdeacido!20, rounded corners=3pt] (0,0) rectangle (0.9,0.6);
        \draw[thick, laranjacel, fill=laranjacel!20, rounded corners=3pt] (1.0,0) rectangle (1.9,0.6);
        \node[below, font=\small] at (0.95,-0.3) {Quaternária};
        \node[above, font=\footnotesize] at (0.95,0.3) {Subunidades};
    \end{scope}
\end{tikzpicture}
\caption{Os quatro níveis de organização estrutural de proteínas. \emph{Primária}: sequência linear de aminoácidos unidos por ligações peptídicas. \emph{Secundária}: padrões locais regulares (mostrados: $\alpha$-hélice). \emph{Terciária}: arranjo tridimensional completo de uma cadeia dobrada. \emph{Quaternária}: associação de múltiplas cadeias em um complexo multimérico --- no exemplo, duas subunidades em interação.}
\label{fig:07-estrutura}
\end{figure}
...
criam sítios de docking para proteínas adaptadoras (Grb2, SOS) e efetoras (PI3K, PLC$\gamma$).
