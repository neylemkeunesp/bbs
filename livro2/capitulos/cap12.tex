...
A Figura~\ref{fig:12-coracao} apresenta a estrutura anatômica do
coração em vista esquemática: as quatro câmaras (átrio direito AD,
átrio esquerdo AE, ventrículo direito VD, ventrículo esquerdo VE),
separadas pelo septo interventricular --- nota-se a parede mais
espessa do VE, que opera contra pressões substancialmente mais
elevadas ($\sim 120$ mmHg vs. $\sim 25$ mmHg no VD).
...
\end{tikzpicture}
\caption{Esquema anatômico do coração em corte --- as quatro câmaras (átrios direito AD e esquerdo AE no topo; ventrículos direito VD e esquerdo VE na base) separadas pelo septo interventricular. A parede do ventrículo esquerdo é substancialmente mais espessa do que a do direito, refletindo a diferença de pressão que cada um deve gerar ($\sim 120$ mmHg na circulação sistêmica vs. $\sim 25$ mmHg na pulmonar). O eixo de fluxo sanguíneo é unidirecional: AD $\to$ VD $\to$ pulmões $\to$ AE $\to$ VE $\to$ aorta.}
\label{fig:12-coracao}
\end{figure}
...
automaticamente à frequência cardíaca. A Figura~\ref{fig:12-niveis}
organiza os níveis de integração do sistema cardíaco: dos canais
iônicos ($\mathrm{Na}^+$, $\mathrm{K}^+$, $\mathrm{Ca}^{2+}$), passando
pelo cardiomiócito, tecido, órgão e sistema cardiovascular --- cada
nível emerge das propriedades do nível inferior e impõe restrições
sobre o nível superior.
...
\end{tikzpicture}
\caption{Pirâmide de níveis de organização do sistema cardíaco, do molecular ao sistêmico. \textbf{Canais iônicos} ($\mathrm{Na}^+$, $\mathrm{K}^+$, $\mathrm{Ca}^{2+}$) determinam o potencial de ação do \textbf{cardiomiócito}; o acoplamento entre cardiomiócitos via gap junctions define a propagação no \textbf{tecido cardíaco}; a integração tecidual produz a função do \textbf{órgão (coração)} como bomba; finalmente, o coração em conjunto com a vasculatura e regulação autonômica forma o \textbf{sistema cardiovascular}. A modelagem matemática em cardiologia é hierárquica --- cada nível requer modelos próprios que se acoplam aos adjacentes.}
\label{fig:12-niveis}
\end{figure}

A Figura~\ref{fig:12-camaras} ilustra o coração esquematicamente em
vista anterior --- as quatro câmaras representadas em retângulos
coloridos, com os átrios (acima) e ventrículos (abaixo). A
Figura~\ref{fig:12-loop-pv} mostra a representação canônica do
ciclo cardíaco: o \emph{loop pressão-volume}, que descreve em duas
dimensões a relação instantânea entre pressão intraventricular e
volume de sangue ao longo das quatro fases (enchimento, contração
...
\end{tikzpicture}
\caption{Loop pressão-volume do ciclo cardíaco. O diagrama representa em duas dimensões a relação entre pressão intraventricular (eixo vertical) e volume ventricular (eixo horizontal) ao longo de um ciclo completo. As quatro fases são: (1) \emph{Enchimento} --- aumento de volume a baixa pressão (segmento horizontal inferior); (2) \emph{Contração isovolumétrica} --- aumento rápido de pressão com volume constante (segmento vertical esquerdo); (3) \emph{Ejeção} --- redução de volume a alta pressão (segmento horizontal superior); (4) \emph{Relaxamento isovolumétrico} --- queda rápida de pressão com volume constante (segmento vertical direito). A área interna do loop é o trabalho sistólico por ciclo.}
\label{fig:12-loop-pv}
\end{figure}
...
chave mecânica do acoplamento temporal entre as câmaras. A
Figura~\ref{fig:12-conducao} esquematiza o sistema de condução: o
nó sinoatrial (SA) no topo do átrio direito atua como marca-passo
natural; o impulso propaga-se pelos átrios até o nó atrioventricular
(AV), que introduz o atraso crucial; segue pelo feixe de His,
ramos (esquerdo e direito) e fibras de Purkinje para distribuir a
ativação pelo endocárdio ventricular.
...
A Figura~\ref{fig:12-ecg} ilustra o traçado típico de um ECG: a onda P
(despolarização atrial), o complexo QRS (despolarização ventricular,
alta amplitude) e a onda T (repolarização ventricular). Os intervalos
PR e QT fornecem leituras quantitativas da condução.
...
\begin{tikzpicture}[scale=0.8]
    ...
    % QRS complex
    \draw[thick, azulescuro] (1.8,0) -- (1.9,0.3) -- (2.0,-0.5) -- (2.2,3) -- (2.4,0);
    \node[above, font=\tiny] at (2.1,3.2) {QRS};

    % T wave
    \draw[thick, azulescuro] (3,0) .. controls (3.2,0.6) and (3.6,0.6) .. (3.8,0);
    \node[above, font=\tiny] at (3.4,0.8) {T};
\end{tikzpicture}
\caption{Traçado típico de um eletrocardiograma (ECG) --- uma derivação. As três ondas fundamentais estão indicadas: \textbf{onda P} --- despolarização atrial ($\sim 80$ ms); \textbf{complexo QRS} --- despolarização ventricular ($\sim 80$ ms, alta amplitude pela massa muscular envolvida); \textbf{onda T} --- repolarização ventricular ($\sim 160$ ms). Os intervalos \textbf{PR} ($0{,}12$--$0{,}20$~s) e \textbf{QT} ($0{,}35$--$0{,}44$~s) são janelas quantitativas para a função de condução.}
\label{fig:12-ecg}
\end{figure}
...
inverte --- o $V_m$ cai rapidamente em direção a $E_K$. A
Figura~\ref{fig:12-pa} apresenta o traçado característico do PA
ventricular, identificando as cinco fases (4--repouso, 0--
despolarização rápida, 1--repolarização inicial, 2--platô, 3--
repolarização final).
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[thick, azulescuro] (1.3,3.5) -- (1.5,1.5);
    \node[right, font=\tiny, red] at (1.5,2) {1};

    % Phase 2 - plateau
    \draw[thick, azulescuro] (1.5,1.5) -- (4,1.4);
    \node[right, font=\tiny, red] at (4,1.8) {2};

    % Phase 3 - final repolarization
    \draw[thick, azulescuro] (4,1.4) -- (5,0);
    \node[right, font=\tiny, red] at (4.7,0.7) {3};
\end{tikzpicture}
\caption{Potencial de ação ventricular --- as cinco fases. \textbf{Fase 4} (repouso): $V_m \approx -90$ mV, mantido por $I_{\mathrm{K1}}$. \textbf{Fase 0} (despolarização rápida): corrente massiva de $\mathrm{Na}^+$ via Nav1.5 eleva $V_m$ a $+30$ mV em $<1$ ms (upstroke do QRS). \textbf{Fase 1} (repolarização inicial): inativação de $I_{\mathrm{Na}}$ + ativação transitória de $I_{\mathrm{to}}$ produzem queda breve. \textbf{Fase 2} (platô): equilíbrio entre $I_{\mathrm{Ca},L}$ (entrada) e $I_{\mathrm{Kr}}$+$I_{\mathrm{Ks}}$ (saída) mantém $V_m$ próximo de 0 mV por $\sim 200$ ms --- base do acoplamento excitação-contração. \textbf{Fase 3} (repolarização final): inativação de $I_{\mathrm{Ca},L}$ + correntes de K$^+$ dominam, e $V_m$ retorna a $E_K$.}
\label{fig:12-pa}
\end{figure}
...
contribuem para arritmias. A Figura~\ref{fig:12-pa-correntes} ilustra o
PA ventricular simultaneamente com as correntes iônicas subjacentes:
$I_{\mathrm{Na}}$ (entrada rápida), $I_{\mathrm{Ca},L}$ (entrada
sustentada), $I_{\mathrm{Kr}}$ (saída durante o platô), $I_{\mathrm{K1}}$
(manutenção do repouso) --- cada uma com seu perfil temporal
característico.
...
\end{tikzpicture}
\caption{Esquema simplificado do PA ventricular (curva azulescura), mostrando a evolução temporal do potencial de membrana $V_m$ durante um ciclo. A rápida despolarização (Fase 0) seguida do platô prolongado (Fase 2) e da repolarização (Fase 3) é a assinatura eletrofisiológica do cardiomiócito ventricular.}
\label{fig:12-pa-correntes}
\end{figure}
...
mais ciclos: existe uma frequência máxima intrínseca. A
Figura~\ref{fig:12-refrat} ilustra o conceito: o primeiro PA
(curva azulescura) é seguido de um período refratário efetivo (ERP)
sombreado em vermelho, durante o qual nenhum estímulo --- por mais
intenso que seja --- consegue deflagrar um novo PA. A parte final do
ERP corresponde ao período refratário relativo, em que estímulo
supra-limiar consegue gerar PA de menor amplitude.
...
\begin{tikzpicture}[scale=0.7]
    ...
    \node[font=\tiny] at (2,1.7) {Período Refratário};
\end{tikzpicture}
\caption{Período refratário efetivo (ERP). Após o término de um potencial de ação (curva azulescura), a célula entra em estado refratário (região sombreada em vermelho) durante o qual não consegue gerar novo PA --- é a base do \emph{período refratário absoluto} ($\sim 150$--$200$ ms). Após este período, segue-se o \emph{período refratário relativo}, em que estímulo mais intenso consegue deflagrar um PA de amplitude reduzida. O ERP é a base da frequência máxima intrínseca do coração.}
\label{fig:12-refrat}
\end{figure}
...
escala humana. A Figura~\ref{fig:12-celula-sr} ilustra a
arquitetura mínima do cardiomiócito para modelagem do acoplamento
excitação-contração: o \textbf{sarcolema} (membrana plasmática) com
canais de $\mathrm{Ca}^{2+}$ tipo L ($I_{\mathrm{Ca},L}$), e o
\textbf{retículo sarcoplasmático (SR)} que armazena e libera
$\mathrm{Ca}^{2+}$ via receptores de rianodina (RyR).
...
\end{tikzpicture}
\caption{Diagrama simplificado do cardiomiócito mostrando os dois compartimentos principais para modelagem do $\mathrm{Ca}^{2+}$: o \textbf{sarcolema} (membrana externa) com canais de $\mathrm{Ca}^{2+}$ tipo L ($I_{\mathrm{Ca},L}$ --- entrada de $\mathrm{Ca}^{2+}$), e o \textbf{retículo sarcoplasmático (SR)} que armazena e libera $\mathrm{Ca}^{2+}$ via RyR (\emph{release}). A recaptação para o SR (\emph{uptake}) é mediada pela bomba SERCA. A integração desses fluxos determina a concentração citosólica de $\mathrm{Ca}^{2+}$ que, por sua vez, controla a contração.}
\label{fig:12-celula-sr}
\end{figure}

A Figura~\ref{fig:12-tradeoff} ilustra o trade-off fundamental em
modelagem cardíaca: o realismo (e o custo computacional) cresce
assintoticamente com a complexidade do modelo, mas há retornos
decrescentes --- a partir de certo ponto, adicionar mais variáveis não
melhora substancialmente a capacidade preditiva. Modelos de cinco
variáveis capturam o PA qualitativo; modelos de trinta ou mais
(O'Hara-Rudy) capturam arritmias e resposta a fármacos.
...
\end{tikzpicture}
\caption{Trade-off entre complexidade do modelo e realismo/custo computacional. A curva verde --- assintótica --- mostra que adicionar variáveis além de certo ponto ($\sim 15$--$20$ variáveis) traz retornos decrescentes em realismo preditivo. O eixo vertical é compartilhado: o mesmo gráfico ilustra tanto o ganho em capacidade de reproduzir fenômenos quanto o custo computacional em FLOPS. A escolha de modelo é ditada pela pergunta: pesquisa qualitativa --- cinco variáveis bastam; previsão quantitativa em escala clínica --- modelos humanos detalhados.}
\label{fig:12-tradeoff}
\end{figure}
...
a miosina, que é o maior consumidor de ATP do ciclo cardíaco. A
Figura~\ref{fig:12-ttub-rsr} ilustra a arquitetura molecular do
acoplamento: o \textbf{T-túbulo} (invaginação do sarcolema) traz o
sinal elétrico para o interior da célula; o canal \textbf{DHPR}
(receptor de dihidropiridina, \emph{L-type Ca channel}) no T-túbulo
detecta a despolarização e abre; o \textbf{RyR} (receptor de rianodina)
na membrana do SR é ativado mecanicamente pelo DHPR e libera
$\mathrm{Ca}^{2+}$ armazenado --- o CICR.
...
\begin{tikzpicture}[scale=0.7]
    ...
    % RyR
    \draw[thick, fill=azulclaro!50] (1.5,-1.7) rectangle (1.8,-2.3);
    \node[below, font=\tiny] at (1.65,-2.5) {RyR};

    % Arrow for Ca release
    \draw[->, thick, red] (1.65,-1.6) -- (1.65,-0.4);
    \node[right, font=\tiny, red] at (1.8,-1) {Ca$^{2+}$};
\end{tikzpicture}
\caption{Acoplamento molecular DHPR-RyR no cardiomiócito. O \textbf{T-túbulo} (acima) é uma invaginação do sarcolema que leva o sinal elétrico para o interior da célula; o \textbf{DHPR} (receptor de dihidropiridina, \emph{L-type Ca channel}) é o sensor de voltagem que detecta a despolarização. O \textbf{RyR} (receptor de rianodina) é o canal de liberação de $\mathrm{Ca}^{2+}$ do retículo sarcoplasmático (abaixo). A seta vermelha indica o fluxo de $\mathrm{Ca}^{2+}$ --- primeiro entrando via DHPR (trigger calcium), depois amplificado pela liberação via RyR (CICR, \emph{calcium-induced calcium release}). A proximidade de $\sim 12$ nm entre DHPR e RyR é crucial para a eficiência do acoplamento.}
\label{fig:12-ttub-rsr}
\end{figure}
...
gerando \emph{afterdepolarizations} e taquicardias. A
Figura~\ref{fig:12-sarcomero} ilustra a organização molecular do
sarcômero: o filamento \textbf{fino} (actina, verde) e o filamento
\textbf{grosso} (miosina, roxo) interdigitados, com as cabeças de
miosina projetando-se em direção aos sítios de ligação da actina. A
hidrólise de ATP nas cabeças de miosina gera o \emph{power stroke}
que desliza os filamentos --- o encurtamento do sarcômero.
...
\end{tikzpicture}
\caption{Esquema simplificado do sarcômero --- unidade contrátil do cardiomiócito. Os filamentos \textbf{finos} de actina (acima, verde) e os filamentos \textbf{grossos} de miosina (abaixo, roxo) estão interdigitados. As cabeças de miosina projetam-se do filamento grosso em direção aos sítios de ligação da actina no filamento fino. A ligação actina-miosina --- disparada pelo $\mathrm{Ca}^{2+}$ ligado à troponina C --- e a hidrólise de ATP subsequente produzem o \emph{power stroke}, deslocando os filamentos e encurtando o sarcômero.}
\label{fig:12-sarcomero}
\end{figure}
...
\begin{exemploenumv}[Lei de Frank-Starling]
...
distendidos (até o limite de $\sim 2{,}2\,\mu\,m$ de sobreposição ótima)
...
A Figura~\ref{fig:12-frank-starling} apresenta a curva clássica de
Frank-Starling --- força ativa em função do comprimento do sarcômero.
O \emph{ramo ascendente} (até $L_0 \approx 2{,}0$--$2{,}2\,\mu\,m$)
reflete o aumento de sobreposição actina-miosina; o \emph{platô} em
uma faixa estreita, a sobreposição ótima; o \emph{ramo descendente}
(acima de $2{,}4\,\mu\,m$) reflete a perda de sobreposição. Em
condições fisiológicas, o coração opera predominantemente no ramo
ascendente, com margem para ajuste por pré-carga.
...
\begin{tikzpicture}[scale=0.8]
    ...
    % Plateau
    \draw[thick, verdeescuro] (2,3) -- (2.4,3);

    % Descending limb
    \draw[thick, red] (2.4,3) -- (3,0);
\end{tikzpicture}
\caption{Curva clássica de Frank-Starling --- força ativa em função do comprimento do sarcômero. O \textbf{ramo ascendente} (azulescuro, $L < L_0$) reflete o aumento de sobreposição actina-miosina --- quanto mais distendido o sarcômero, mais pontes cruzadas se formam. O \textbf{platô} (verde-escuro, $L \approx L_0$ a $1{,}2 L_0$) marca a sobreposição ótima. O \textbf{ramo descendente} (vermelho, $L > 1{,}2 L_0$) reflete a perda de sobreposição e queda de força. A lei de Frank-Starling --- \emph{a força é proporcional ao comprimento inicial} --- explica o aumento automático do débito sistólico em resposta a aumento de pré-carga.}
\label{fig:12-frank-starling}
\end{figure}
...
extinguida. A Figura~\ref{fig:12-reentry} ilustra o circuito de
reentrada: uma onda espiral gira em torno de um obstáculo central,
com bloqueio unidirecional --- o tecido é refratário na direção
retrógrada mas excitável na direção anterógrada, permitindo a
persistência indefinida da frente de onda.
...
\end{tikzpicture}
\caption{Circuito de reentrada em uma região com obstáculo central (região cinza). A frente de onda (azul) gira em torno do obstáculo, formando uma espiral. A condição para sustentação é que o comprimento de onda $\lambda = \theta \times \mathrm{ERP}$ seja menor que o perímetro do circuito --- caso contrário, a frente encontra tecido em refratura ao retornar e é extinta. O bloqueio unidirecional é a chave: o tecido é refratário em uma direção, excitável na outra, permitindo a reentrada.}
\label{fig:12-reentry}
\end{figure}
...
prevenir a cronificação. A Figura~\ref{fig:12-fibrilacao}
ilustra o padrão de ativação caótica: múltiplas espirais (rotores)
coexistem no tecido atrial, com frentes de onda fragmentadas e
dessincronizadas --- a assinatura eletrofisiológica da fibrilação
atrial.
...
\end{tikzpicture}
\caption{Ativação caótica característica de fibrilação atrial. Múltiplas espirais (rotores) --- três representadas no exemplo --- coexistem no tecido, com frentes de onda fragmentadas e colidindo umas com as outras. Esta é a assinatura eletrofisiológica da FA, distinguindo-a de taquiarritmias organizadas (que mantêm um único circuito de reentrada). A complexidade espacial e temporal --- $\sim 400$--$600$ batimentos por minuto --- impede contração atrial coordenada.}
\label{fig:12-fibrilacao}
\end{figure}
...
prolongamento QT a partir de perfis de bloqueio iônico. A
Figura~\ref{fig:12-lqts} compara o potencial de ação normal (verde)
com o PA na síndrome do QT longo (vermelho), este com uma
\emph{early afterdepolarization} (EAD) durante a Fase 2 --- uma
segunda despolarização que pode deflagrar \emph{torsades de
pointes} e fibrilação ventricular.
...
\begin{tikzpicture}[scale=0.6]
    ...
    \end{scope}
    \node[left, font=\tiny] at (0,-0.3) {LQTS};
\end{tikzpicture}
\caption{Comparação entre o potencial de ação normal (curva verde, topo) e o PA na síndrome do QT longo (curva vermelha, abaixo). Na LQTS, o prolongamento do platô e a inativação heterogênea de $I_{\mathrm{Kr}}$ (ou $I_{\mathrm{Ks}}$) permitem o reaparecimento transitório de corrente de entrada --- tipicamente $I_{\mathrm{Ca},L}$ reativado --- que gera uma \emph{early afterdepolarization} (EAD, segundo pico em forma de corcunda na curva vermelha). A EAD pode atingir o limiar de disparo e deflagrar \emph{torsades de pointes}, que pode degenerar em fibrilação ventricular.}
\label{fig:12-lqts}
\end{figure}

\begin{exemploenumv}[Modulação da velocidade de propagação por drogas]
