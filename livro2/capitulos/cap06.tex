...
\centering
\begin{tikzpicture}[
    box/.style={rectangle, draw, text width=2.4cm,
...
\end{tikzpicture}
\caption{Dogma central da biologia molecular, formulado por Crick em 1958. O fluxo canônico de informação genética vai do DNA ao RNA por \emph{transcrição}, e do RNA à proteína por \emph{tradução}. A \emph{replicação} do DNA é o mecanismo que permite a transmissão fiel da informação genética ao longo das gerações. Exceções notáveis --- como a transcrição reversa em retrovírus --- não invalidam o dogma, mas o complementam.}
\label{fig:06-dogma}
\end{figure}
...
\begin{tikzpicture}[scale=0.85, every node/.style={font=\small}]
    ...
    % Promotor
    \fill[roxodna!50] (0.5,-0.3) rectangle (1.5,0.3);
    \node[above, font=\small] at (1,0.3) {P};

    % Operador
    \fill[red!50] (1.7,-0.3) rectangle (2.7,0.3);
    \node[above, font=\small] at (2.2,0.3) {O};

    % Genes
    \fill[verdeacido!50] (3.2,-0.3) rectangle (4.7,0.3);
    \node[above, font=\small] at (3.95,0.3) {\textit{lacZ}};
    \fill[verdeacido!50] (5.2,-0.3) rectangle (6.7,0.3);
    \node[above, font=\small] at (5.95,0.3) {\textit{lacY}};
    \fill[verdeacido!50] (7.2,-0.3) rectangle (8.7,0.3);
    \node[above, font=\small] at (7.95,0.3) {\textit{lacA}};

    % Repressor bloqueando
    \node[draw, circle, fill=yellow!50, minimum size=0.6cm, font=\footnotesize] at (2.2,-1.5) {LacI};
    \draw[->, very thick, red] (2.2,-1.1) -- (2.2,-0.4);

    % Lactose
    \node[draw, circle, fill=azulclaro!50, minimum size=0.6cm, font=\footnotesize] at (0.5,-1.5) {Lac};
    \draw[->, dashed, azulescuro] (0.9,-1.5) -- (1.7,-1.5);

    % Status
    \node[below, font=\small, red!70!black] at (5,-2.3) {Sem lactose: LacI bloqueia o operador (OFF)};
\end{tikzpicture}
\caption{Operon \textit{lac} de \textit{Escherichia coli} no estado reprimido. O repressor LacI (amarelo) liga-se ao operador (O, vermelho), impedindo fisicamente a transcrição dos genes estruturais \textit{lacZ}, \textit{lacY}, \textit{lacA} pela RNA polimerase. A lactose (azul) --- quando presente --- liga-se a LacI e induz a dissociação do repressor, permitindo a transcrição.}
\label{fig:06-operon}
\end{figure}
