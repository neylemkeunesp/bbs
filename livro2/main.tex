...
% Regra: todo diagrama TikZ deve ter fundo branco opaco, garantindo
% legibilidade quando o leitor usa fundo escuro (dark mode) no PDF.
% Desenhamos um retângulo branco do tamanho da bounding box na camada de
% background (atrás dos nodes), via execute at end picture + scope on
% background layer. O retângulo vai ATRÁS do conteúdo, não por cima.
\tikzset{every picture/.append style={execute at end picture={\begin{scope}[on background layer]
  \path[fill=white,fill opacity=1] (current bounding box.south west) rectangle (current bounding box.north east);
\end{scope}}}}
\usepackage{chemfig}
\usepackage{listings}
...
% Texto em math mode: usar \mb{ion} em vez de "ion" puro (evita itálico matemático)
\newcommand{\mb}[1]{\mathrm{#1}}
...
\include{capitulos/cap06}
\include{capitulos/cap07}
\include{capitulos/cap08}
\include{capitulos/cap09}
